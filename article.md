---
<!-- TODO: best translation for "parsing"? -->
title: "Rust и разбор CSV"
categories: руководства
author: Andrew Gallant
translator: Борис Егоров
---

С релизом бета-версии `csv 1.0`, пришло время урока по чтению и записи данных в формате CSV в Rust. Урок нацелен на начинающих программистов на Rust, и поэтому полон примеров и подробно разбирает основы. Опытные программисты на Rust могут найти для себя полезными некоторые части, но вероятнее будут рады пробежаться глазами. (TODO, звучит не очень)

Введение в Rust можно найти в [официальной книге](https://doc.rust-lang.org/beta/book/second-edition/).  Если вы ещё не писали кода на Rust, но использовали другой язык, урок может быть доступным для вас и без чтения книги.

Библиотека CSV [доступна на Github](https://github.com/BurntSushi/rust-csv) и имеет [исчерпывающую документацию API](https://docs.rs/csv).

Наконец, версия этого поста включена в [руководство](https://docs.rs/csv/1.0.0-beta.3/csv/tutorial/index.html) в документации API, которое скорее всего будет постепенно обновляться.

**Целевая аудитория**: начинающие программисты на Rust.

<!-- Habr: first, rustycrate: second -->
<cut/>
<!--cut-->

## Содержание

* [Релиз csv 1.0](#rieliz-csv-10)
* [Подготовка к работе](#podghotovka-k-rabotienastroika-okruzhieniia)
* [Основы обработки ошибок](#osnovy-obrabotki-oshibok)
    * [Переход к исправимым ошибкам](#pieriekhod-k-ispravimym-oshibkam)
* [Чтение CSV](#chtieniie-csv)
    * [Чтение заголовков](#reading-headers)
...
Перед началом урока расскажу вкратце о дороге к 1.0. Первый коммит в репозиторий `rust-csv` появился 22 марта 2014, чуть больше года перед релизом Rust 1.0. Те, кто сталкивался с Rust до 1.0, определённо помнят как сильно изменился язык. Конечно, изменился и я, узнал лучше язык и его идиомы. Тем не менее, API библиотеки CSV в основном осталось тем же с изначальной версии. API затруднил повышение производительности и исправление нескольких незаурядных багов, и хуже всего, использовал старый подход к сериализации.

Релиз CSV 1.0 отмечает более быструю библиотеку, с лучшим API и поддержкой [Serde](https://serde.rs), фреймворка сериализации Rust.

Новая библиотека CSV поставляется с контейнером [`csv-core`](https://docs.rs/csv-core), который может разбирать данные в формате CSV без стандартной библиотеки Rust и в основном ответственен за повышение производительности. В частности, старая библиотека CSV использовала тип конечного автомата с большими накладными расходами. Контейнер `csv-core` компилирует парсер в детерминированный конечный автомат на основе таблицы, занимающий всего несколько сотен байт на стеке. В результате получено ускорение примерно в два раза повсюду:
...
В этом разделе мы настроим окружение и проверим его с помощью простой программы, которая читает данные в формате CSV и печатает "отладочную" версию каждой записи. Для работы у вас должен быть установлен [набор инструментов Rust](https://www.rust-lang.org/install.html), включающий Rust и Cargo.

Начнём с создания нового проекта Cargo:
...
Откроем файл `Cargo.toml` в директории `csvtutor` и добавим `csv = "1.0.0-beta.3"` в секцию `[dependencies]` (зависимости). После этого файл `Cargo.toml` должен выглядеть примерно так:
...
Теперь соберём проект. Поскольку мы добавили контейнер `csv` в зависимости, Cargo автоматически скачает и соберёт его. Для сборки проекта используем Cargo:

```text
$ cargo build
```

Эта команда соберёт бинарный файл `csvtutor` в директории `target/debug`. Он ещё не делает ничего полезного, но можно его запустить:
...
Теперь сделаем что-нибудь полезное. Программа будет читать данные в формате CSV из stdin (TODO: стоит перевести?) и печатать отладочный вывод в stdout.  Откроем файл `src/main.rs` и поменяем его содержимое на следующее:

```rust
//tutorial-setup-01.rs
// Делаем контейнер csv доступным программе.
extern crate csv;

// Импортируем модуль ввода-вывода стандартной библиотеки для чтения из stdin.
use std::io;
...
fn main() {
    ...
    for result in rdr.records() {
        // Может произойти ошибка, при этом прерываем программу не дружелюбным способом.
        // Мы это исправим позднее!
        let record = result.expect("a CSV record");
        //  Печатаем отладочную версию записи.
        println!("{:?}", record);
    }
}
```

Пока что не задумывайтесь, что значит этот код; мы разберём его в следующей части. Сейчас пересоберём проект:

```text
$ cargo build
```

После успешной сборки запустим программу. Но для начала нам понадобятся какие-нибудь CSV-данные (TODO: ok?) на поиграться (FIXME)! Используем в качестве них случайную выборку 100 городов России с их населением и географическими координатами. (Мы будем использовать эти данные на протяжении всего урока.) Скачиваем данные с github:

```text
$ curl -LO 'https://raw.githubusercontent.com/BurntSushi/rust-csv/master/examples/data/uspop.csv'
```

И наконец, запускаем программу на `uspop.csv`:
...
Поскольку чтение CSV-данных может привести к ошибкам, обработка ошибок будет сопровождать все примеры в этом уроке. Начнём с рассмотрения основ обработки ошибок, в частности, исправим предыдущий пример, чтобы ошибки отображались более дружественно. **Если вы уже работали с `Result` и `try!`/`?` в Rust, можете смело пропускать этот раздел.**

(TODO: сменить ссылки на перевод?)
Отметим, что [книга The Rust Programming Language](https://doc.rust-lang.org/beta/book/second-edition/)
содержит [введение в (обобщённую TODO?) обрабоку ошибок](https://doc.rust-lang.org/beta/book/second-edition/ch09-00-error-handling.html).
Более детально тему рассматривает [мой пост об обработке ошибок в Rust](http://blog.burntsushi.net/rust-error-handling/). Этот пост особенно важен, если вы планируете писать библиотеки.

Перейдём же к теме (TODO: ok?). Ошибки в Rust имеют две формы: исправимые ошибки и неисправимые.

Неисправимые ошибки обычно соответствуют багам в программе, которые происходят при нарушении инварианта или контракта. В этот момент состояние программы непредсказуемо, и обычно лучшим выходом является *паника*. В Rust паника подобна простому прерыванию программы, но с раскруткой стека и освобождением ресурсов перед выходом программы.

С другой стороны, исправимые ошибки обычно соответсвтуют предсказуемым ошибкам. К таким относятся, например, несуществующий файл или невалидные CSV-данные. В Rust исправимые ошибки представляются типом `Result`. `Result` представляет состояние вычислений, которое завершилось успешно или не удалось. Он определён следующим образом:
...
То есть, `Result` содержит значение типа `T` когда вычисление успешно завершилось, или значение типа `E` в случае ошибки.

Взаимодействие между неисправимыми ошибками и исправимыми важно. В частности, **настоятельно не рекомендуется** обращаться с исправимыми ошибками как с неисправимыми. Например, паника при отсутствии файла или невалидных CSV-данных считается плохой практикой. Вместо этого, предсказуемые ошибки должны представляться типом `Result`.

Вооружившись этими знаниями, взглянем снова на предыдущий пример и обработку ошибок в нём.
...
Есть два места, в которых может произойти ошибка в этой программе. Первое - проблема чтения строки из stdin. Вторая - проблема записи в stdout. Вторую проблему мы проигнорируем в этом уроке, хотя надёжные приложения с интерфейсом командной строки вероятно должны обрабатывать её (к примеру, когда происходит нарушение канала (TODO: broken pipe)). Заострим внимание на первой проблеме. Когда пользователь передаёт программе невалидные CSV-данные, программа паникует:
...
Что здесь происходит? В первую очередь, почему данные не валидны? Здесь данные состоят из трёх записей: заголовка и двух записей данных. Заголовок и первая запись имеют два поля, а вторая запись - три поля. По умолчанию контейнер csv рассматривает неконсистентные длины записей как ошибку. (Это поведение переключается с помощью опции [`ReaderBuilder::flexible`](https://docs.rs/csv/1.0.0-beta.3/csv/struct.ReaderBuilder.html#method.flexible).)
Вот почему первая запись печатается: она содержит столько же полей, как и заголовок. То есть, мы не встречаем ошибку до момента разбора второй записи.

(Заметим, что читатель CSV автоматически интерпретирует первую запись как заголовок. Это настраивается опцией [`ReaderBuilder::has_headers`](https://docs.rs/csv/1.0.0-beta.3/csv/struct.ReaderBuilder.html#method.has_headers).)

Что же заставляет программу паниковать (TODO: вызывать/входить в состояние паники)? Первая строка в цикле:

```rust
for result in rdr.records() {
    let record = result.expect("a CSV record"); // вызывает панику
    println!("{:?}", record);
}
```

Главное, что следует понять: `rdr.records()` возвращает итератор, который производит значения типа `Result`. То есть, вместо производства (TODO) записей, он производит (TODO) `Result`, содержащий запись или ошибку. Метод `expect`, определённый на `Result`, *разворачивает* (TODO) успешное значение из `Result`. Если `Result` вместо этого содержит ошибку, `expect` вызовет *панику*.

Для понимания можно взглянуть на реализацию `expect`:

```rust
use std::fmt;

// Сигнатура значит: "для всех типов T и E, где E можете быть представлено как
// человекочитаемое отладочное сообщение, определить метод `expect`."
...
Так как паника вызывается при невалидных CSV-данных, а невалидные CSV-данные совершенно предсказуемая ошибка, мы превратили *исправимую* ошибку в *неисправимую*. Мы это сделали, потому что неисправимые ошибки целесообразно использовать (TODO: wat?). Поскольку это плохая практика, мы постараемся избегать неисправимых ошибок в дальнейшем.

## Переход к исправимым ошибкам

Мы перейдём нашу неисправимую ошибвку в исправимую в 3 шага. Сначала избавимся от паники и напечатаем сообщение об ошибке вручную:
...
fn main() {
    ...
    for result in rdr.records() {
        // Проверим Result.
        // В случае отсутствия ошибок распечатаем запись.
        // Иначе покажем сообщение об ошибке и завершим программу.
        match result {
...
Если мы снова запустим программу, мы всё ещё будем видеть сообщение об ошибке, но не о панике:
...
Второй шаг для перехода к исправимым ошибкам - перенести цикл по записям CSV в отдельную функцию. Эта функция сможет *вернуть* ошибку, которую проверит `main` и решит, что делать дальше.
...
fn run() -> Result<(), Box<Error>> {
    ...
    for result in rdr.records() {
        // Проверим Result.
        // В случае отсутствия ошибок распечатаем запись.
        // Иначе сконвертируем ошибку в Box<Error> и вернём.
        match result {
...
Новая функция `run` возвращает значение типа `Result<(), Box<Error>>`. Проще говоря, она возвращает ничто при успешном завершении или `Box<Error>` (что значит "любая ошибка") в случае, если произошла ошибка. `Box<Error>` сложно проверить на конкретный тип ошибки, но для наших целей он подходит. Мы просто распечатаем сообщение об ошибке и завершим программу.

Третий и последний шаг заключается в замене явного выражения `match` специальной возможностью языка Rust: вопросительным знаком.
...
fn run() -> Result<(), Box<Error>> {
    let mut rdr = csv::Reader::from_reader(io::stdin());
    for result in rdr.records() {
        // Код делает практически то же, что делал `match` в прошлом примере.
        // Другими словами, `?` является синтаксическим сахаром.
        let record = result?;
        println!("{:?}", record);
    }
    Ok(())
...
Последний шаг показывает, как мы можем использовать `?` для автоматического проброса (TODO: forward) ошибок вызывающей стороне без необходимости явного рассмотрения случаев `match`. Далее мы будем активно использовать `?`, поэтому отметим важную деталь: **вопросительный знак может использоваться только в функциях, возвращающих `Result`.**

Закончим эту часть предупреждением: использование `Box<Error>` в качестве типа ошибки минимально приемлемый подход в данном случае. Он позволяет корректно обработать ошибки, но лишает вызывающую сторону возможности узнать, какая именно ошибка случилась. Поскольку это урок по созданию программ с интерфейсом командной строки, разбирающих (TODO: parse) CSV, будем считать что мы удовлетворены. Если хотите узнать больше или заинтересованы в написании библиотеки, работающей с CSV, вам стоит посмотреть мой [пост об обработке ошибок](http://blog.burntsushi.net/rust-error-handling/).

Наконец, если вы пишете одноразовую программу для преобразований CSV, использовать методы вроде `expect` и вызывать панику при ошибке совершенно нормально. Но в уроке мы всё же постараемся писать код идиоматично.

## Чтение CSV

С настройкой окружения и основами обработки ошибок разобрались, пора заняться делом: обработкой CSV-данных. Мы уже видели, как читать такие данные из `stdin`, а в этой части почитаем из файлов и посмотрим как настроить читатель (TODO: reader) на разные разделители и способы цитирования.

Сначала адаптируем пример, с которым работали раньше, чтобы он принимал в качестве аргумента путь к файлу с данными.
...
/// Возвращает первый позиционный аргумент, переданный процессу.
/// Если позиционных аргументов нет, возвращает ошибку.
...
Заменим содержимое файла `src/main.rs` представленным выше кодом,
пересоберём и попробуем запустить:
...
Этот пример содержит два новых фрагмента кода:

1. Код для запроса позиционных аргументов программы. Мы поместили этот код в отдельную функцию `get_first_arg`. Программа ожидает, что первым аргументом (с индексом `1`; аргмент с индексом `0` - имя исполняемого файла), поэтому если аргумента нет, возвращается ошибка.
2. Код для открытия файла. В `run` мы открываем файл с помощью `File::open`. Если произошла проблема, мы пробрасываем (TODO: forward) ошибку вызывающей стороне (в данном случае `main`). Отметим, что мы *не* оборачиваем `File` в буфер. Читатель (TODO: reader) CSV буферизует данные внтури, поэтому вызывающей стороне это делать не нужно.

Самое время представить альтернативный конструктор читателя (TODO: reader) CSV, который более удобен для чтения данных из файла. Вместо:
...
`csv::Reader::from_path` откроет файл за вас и вернёт ошибку, если файл не может быть открыт.

## Чтение заголовков

TODO: uspop.csv => rupop.csv
Если вы заглядывали в файл `uspop.csv`, вы могли увидеть запись-заголовок вида:

```text
City,State,Population,Latitude,Longitude
```

Если теперь посмотреть на вывод команд до этого момента, вы заметите, что заголовок не печатается. Почему? По умолчанию, читатель (TODO: reader) CSV интерпретирует первую запись CSV-данных как заголовок, который обычно отличается от самих данных в следующих записях. Поэтому заголовок всегда пропускается при чтении или итерировании по записям CSV.

Читатель (TODO: reader) CSV не пытается быть умным насчёт заголовка и **не** применяет никаких эвристик для автоматического определения заголовка. Если вы не хотите считать первую запись заголовком, нужно явно сказать об этом читателю CSV (TODO: reader).

Для настройки читателя (TODO: reader) CSV нам нужно использовать [`ReaderBuilder`](https://docs.rs/csv/1.0.0-beta.3/csv/struct.ReaderBuilder.html), который конструирует читателя (TODO: reader) с нужной конфигурацией. Например (заметим, что мы снова читаем из `stdin`, так примеры будут компактнее):
...
После сборки и запуска программы на файле `uspop.csv` мы увидим, что заголовок теперь печатается:
...
Если вам когда-нибудь понадобиться обратиться к заголовку напрямую, можете использовать метод [`Reader::header`](https://docs.rs/csv/1.0.0-beta.3/csv/struct.Reader.html#method.headers) вот так:
...
fn run() -> Result<(), Box<Error>> {
    let mut rdr = csv::Reader::from_reader(io::stdin());
    {
        // Этот вызов находится в своей области видимости из-за времён жизни.
    ...
    // Можно запросить заголовки в любое время. Можно не оборачивать этот
    // вызов в область видимости, поскольку больше заимствовать читатель (TODO: reader) мы не будем.
    let headers = rdr.headers()?;
    println!("{:?}", headers);
    Ok(())
}
